...
# def get_html_output_path(url, output_root):
#     parsed = urlparse(url)
#     path = parsed.path.strip("/")

#     if not path:
#         folder_path = os.path.join(output_root)
#         file_name = "index.html"
#     else:
#         parts = path.split("/")
#         *folders, last_part = parts
#         folder_path = os.path.join(output_root, *folders)
#         file_name = f"{last_part}.html"
...
CSV_FILE = os.path.join(OUTPUT_ROOT, "crawl_assets.csv")
COOKIE_FILE = "executables/cookies.json"
STORAGE_FILE = "executables/storage.json"
MAX_PAGES = 50
graph_edges = defaultdict(set)
...
def extract_internal_links(base_url, page):
    ...
    for link in links:
        ...
        if parsed_path.startswith("/-/"):
            continue  # Skip static or media-style paths
...
def crawl_site(playwright):
    ...
    while queue and len(visited) < MAX_PAGES:
        ...
        try:
            ...
            file_path = get_html_output_path(url, OUTPUT_ROOT)
